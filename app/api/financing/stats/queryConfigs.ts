export const QUERY_CONFIGS = {
  ...
  recenttrades: {
    query: (conditions: string[]) => `SELECT * FROM risk_f_mv FINAL ${conditions.length ? 'WHERE ' + conditions.join(' AND ') : ''}`,
    processResult: (data: any) => data
  },
  countrecenttrades: {
    query: (conditions: string[]) => `SELECT COUNT(*) FROM risk_f_mv FINAL ${conditions.length ? 'WHERE ' + conditions.join(' AND ') : ''}`,
    processResult: (data: any) => data
  },
  desk: {
