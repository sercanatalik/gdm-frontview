...
import { FilterSchema, type Measure, type QueryResult } from "./types"
...
export async function fetchMeasureTotal<T>(measure: Measure, filterStr: string | null): Promise<T> {
  try {
    if (!QUERY_CONFIGS[measure]) {
      throw new Error(`Invalid measure: ${measure}`)
    }

    const filter = filterStr ? FilterSchema.parse(JSON.parse(filterStr)) : null
