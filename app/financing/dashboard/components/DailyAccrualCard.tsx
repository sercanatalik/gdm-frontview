...
export function DailyAccrualCard() {
  ...
  useEffect(() => {
    const fetchDailyAccrualAmount = async () => {
      setIsLoading(true); // Add this line
      try {
        const response = await fetch("/api/financing/stats?measure=dailyaccrual");
        if (!response.ok) {
        ...
        console.error("Error fetching daily accrual amount:", error);
      } finally {
        setIsLoading(false); // Add this line
      }
    };

    fetchDailyAccrualAmount();
  }, []);
