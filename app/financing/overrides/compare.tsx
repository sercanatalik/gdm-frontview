          ...
          {keys.map(key => (
            <li key={key} className="flex items-center space-x-2">
              ...
              <span className="font-mono">
                ...
                {title === 'Modified Keys' && ` → ${formatValue(obj2[key])}`}
              </span>
            </li>
          ))}
        </ul>
    ...
    <Card >
      ...
      <CardContent>
        <ScrollArea className="h-full w-full rounded-md p-1">
          {renderDiffSection('Added Keys', differences.added, 'bg-green-500')}
          {renderDiffSection('Removed Keys', differences.removed, 'bg-red-500')}
          {renderDiffSection('Modified Keys', differences.modified, 'bg-orange-500')}
